...
let groupSize = 5;
let url = new URL(
  `https://main--sparkly-vacherin-77cd1f.netlify.app/top-headlines?country=kr&apiKey=${API_KEY}`
);
...
const getLatestNews = async () => {
  url = new URL(
    `https://main--sparkly-vacherin-77cd1f.netlify.app/top-headlines`
...
async function getnewsbycategory(event) {
  ...
  url = new URL(
    `https://main--sparkly-vacherin-77cd1f.netlify.app/top-headlines?category=${category}`
  ); //URL인스턴스로 다양한 함수를 사용가능. (객체가생김)
  getNews();
}
...
const getnewByKeyword = async () => {
  ...
  url = new URL(
    `https://main--sparkly-vacherin-77cd1f.netlify.app/top-headlines?&q=${keyword}`
  );
  getNews();
};
